...
!
      INTEGER*2 NAAA, NBBB, NCCC, NFFF, NHHH, NLLL
...
!
      IF (IPRE .GE. 2) CALL OUTDAT (SWULL)
!
...
!
      IF (IOS .EQ. 1) CALL OUTAV
!
!     CALLS SUBROUTINE OUTPEK TO PRINT PEAK
!     RESULTS FOR THE SIMULATION PERIOD.
!
      IF (IOS .EQ. 1) CALL OUTPEK
      CALL OUTSW
!
      CLOSE (8)
!
...
!
 ...
 1000 CONTINUE
      TAVG(1) = TMPF(IDA)
!
      IF (TMPF(IDA) .LT. 32.0 .AND. KCNT .GT. 0) KCNT = KCNT - 1
      IF (TMPF(IDA) .GE. 32.0 .AND. KCNT .LT. MXKCNT) KCNT = KCNT + 1
!
      IF (IFREZ .EQ. 0) THEN
        IF (TMPAVG .LT. 32.0 .AND. KCNT .EQ. 0) THEN
          IFREZ = 1
          MXKCNT = IDFS
        END IF
      ELSE
...
!
      ...
      CHARACTER*40 CITY4, CITY7, CITY13, CITY11
      CHARACTER*60 F4, F7, F13, F11, F10, F8, TITLE
      INTEGER*2 NH, NMIN, NS, NHD, ND2, NM, NY
