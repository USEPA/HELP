...
!    IYR IS THE NUMBER OF THE YEAR
!    LMYR IS THE NUMBER OF YEARS OF SIMULATION
!    OSNO IS THE INITIAL SNOW WATER
!    AREA IS AREA PROJECTED ON HORIZONTAL PLANE
!
!******************************MAIN*************************
!                         OUTPUT ROUTINE
!
...
!
!    IU8 is a flag to indicate the system of units used for the inputs and
!        outputs. A value of 2 is used for SI and 1 for IP.
!    TFSOIL is the average air temperature, in Fahrenheit degrees,
!           below which the soil is assumed to be freezing
!

      SUBROUTINE run_simulation (fpath_precip, fpath_tasavg, &
       fpath_solrad, fpath_evapo, fpath_soil, fpath_output, &
       IOD, IOM, IOA, IOS, IU8, LMYR, TFSOIL)
!
      PARAMETER (MXYR = 100)
      PARAMETER (MAXMO = 12)
...
!
      IF (IPRE .GE. 2) THEN
         CALL OUTDAT (SWULL, fpath_precip, fpath_tasavg, fpath_solrad, &
                      fpath_evapo, fpath_soil, fpath_output, IU8)
      END IF
!
      ...
      IF (NSEG .EQ. NSEGE) THEN
         ...
         ELSE IF (IBAR .EQ. 2) THEN
            IF (SUBIN(LP(NPROF)) .GT. 0.0 .OR. &
              SUBIN(LP(NPROF)-1) .GT. 0.0) IBAR = IBAR + 3
...
!
      SUBROUTINE OUTDAT (SWULL, fpath_precip, fpath_tasavg, &
       fpath_solrad, fpath_evapo, fpath_soil, fpath_output, IU8)
...
!
 ...
 6000 FORMAT(1X,A1/1X,78('*')/ &
      ...
      WRITE (8, 6005) fpath_precip, fpath_tasavg, fpath_solrad, &
       fpath_evapo, fpath_soil, fpath_output
 6005 FORMAT(///1X,'PRECIPITATION DATA FILE:    ',A260/ &
       1X,'TEMPERATURE DATA FILE:      ', A260/ &
       1X,'SOLAR RADIATION DATA FILE:  ', A260/ &
       1X,'EVAPOTRANSPIRATION DATA:    ', A260/ &
       1X,'SOIL AND DESIGN DATA FILE:  ', A260/ &
       1X,'OUTPUT DATA FILE:           ', A260///)
!
...
!    COMPILER CORRECTIONS BELOW
           IF (LD(L) .gt. 0 .and. LAYR(LD(L)) .EQ. I .AND. RECIR(LD(L)) &
             .GT. 0.0) WRITE (8,6159) RECIR(LD(L)), LD(L)
!     COMPILER CORRECTIONS ABOVE
 6159      FORMAT(10X, &
 ...
 6143      FORMAT(10X, &
         ...
         IF (I .EQ. 1 .AND. ISOIL (1) .LE. 34 .AND. ISOIL (1) .GE. 1 &
           .AND. ULAI .GT. 0.0) WRITE(8,6133) CORECT
