...
- TransactionTestCase but also launch a live HTTP
    server in a separate thread so that the tests may use another testing
    framework, such as Selenium for example, instead of the built-in dummy
    client.
- Define a common 'lockfile' for each set of TestCases to serialize
"""
...
def to_list(value):
    """Put value into list if not already one. Return empty list value None."""
    if value is None:
        value = []
...
class _CursorFailure:
    ...
    def __call__(self):
        raise AssertionError(
            "Database queries are not allowed in SimpleTestCase. "
            "Either use TestCase or TransactionTestCase to ensure proper "
            "test isolation or "
...
class SimpleTestCase(unittest.TestCase):
    """Test client self.client, can be overridden in derived Classes."""
...
def settings(self, **kwargs):
    """Context mgr temp sets setting reverts original value exiting context."""
...
def assertredirects(self, response, expected_url, status_code=302,
    ...
    """
    Assert response redirected to a specific URL and redirect URL can be loaded.
    ...
    if hasattr(response, 'redirect_chain'):
        ...
        if fetch_redirect_response:
            ...
            self.assertEqual(
                ...
                msg_prefix + "could not retrieve redirection page '%s': "
                             "response code was %d (expected %d)"
...
def _assert_contains(self, response, text, status_code, msg_prefix, html):
    ...
    if hasattr(response, 'render') and callable(
            response.render) and not response.is_rendered:
        response.render()
    ...
    self.assertEqual(
        response.status_code, status_code,
        msg_prefix + "could not retrieve content: Response code was %d"
    ...
    if html:
        content = assert_and_parse_html(self, content, None,
                                        "Response's content is not valid"
                                        " HTML:")
...
def assertcontains(self, response, text, count=None, status_code=200,
    ...
    (i.e., the HTTP status code was as expected) and that
...
def assertnotcontains(self, response, text, status_code=200, msg_prefix='',
    ...
    self.assertEqual(real_count, 0,
                     msg_prefix + "Response should not contain %s" %
                     text_repr)


def assertformerror(self, response, form, field, errors, msg_prefix=''):
    """Assert form used to render the response has a specific field error."""
    if msg_prefix:
        msg_prefix += ": "
    ...
    if not contexts:
        self.fail(
            msg_prefix + "Response did not use any contexts to render "
                         "the response")
        ...
        for err in errors:
            if field:
                if field in context[form].errors:
                    ...
                    self.fail(
                        msg_prefix + "The form '%s' in context %d does "
                                     "not contain the field '%s'" %
                        (form, i, field)
                    )
            else:
    ...
    if not found_form:
        self.fail(
            msg_prefix + "The form '%s' was not used to render the "
                         "response" % form)
        ...
        for err in errors:
            if field is not None:
                if field in context[formset].forms[form_index].errors:
                    ...
                    self.fail(
                        msg_prefix + "The field '%s' on formset '%s', "
                                     "form %d in context %d contains no "
                                     "errors"
...
def _assert_raises_message_cm(self, expected_exception, expected_message):
    with self.assertRaises(expected_exception) as con_man:
        yield con_man
...
class TransactionTestCase(SimpleTestCase):
    ...
    Can ask for resetting auto increment sequence before each test case.
    """
    ...
    # If transactions are not available, Django will serialize the database
        ...
        * If the class has a 'fixtures' attribute, install those fixtures.
        ...
        if self.available_apps is not None:
            ...
            for db_name in self._databases_names(include_mirrors=False):
                emit_post_migrate_signal(verbosity=0, interactive=False,
                                         db=db_name)
        try:
            self._fixture_setup()
        except Exception:
            ...
            raise
        # Clear the queries_log so that it is less likely to overflow (a single
        # test probably won't execute 9K queries). If queries_log overflows,
        # then assertNumQueries() does not work.
        for db_name in self._databases_names(include_mirrors=False):
...
class TestCase(TransactionTestCase):
    """
    Similar to TransactionTestCase.

    Use `transaction.atomic()` to achieve test isolation. In most situations,
    TestCase should be preferred to TransactionTestCase allows faster
    execution. However, some situations where using TransactionTestCase might
    be necessary (e.g. testing some transactional behavior).
    ...
    @staticmethod
    def _should_check_constraints(conn):
        return (
                conn.features.can_defer_constraint_checks and
                not conn.needs_rollback and conn.is_usable()
        )
...
class QuietWSGIRequestHandler(WSGIRequestHandler):
    ...
    Does not log to standard output any of the requests received, so as to
    not clutter the test result output.
    """
...
class FSFilesHandler(WSGIHandler):
    ...
    Intercepts calls to a directory, as defined by one of the *_ROOT settings,
    and serves those files, publishing them under *_URL.
    """
...
class _StaticFilesHandler(FSFilesHandler):
    ...
    A private class that is meant to be used solely as a convenience by
    LiveServerThread.
    """
...
class _MediaFilesHandler(FSFilesHandler):
    ...
    A private class that is meant to be
...
class LiveServerTestCase(TransactionTestCase):
    ...
    TestCase because the threads don't share the same transactions (unless if
    using in-memory sqlite) and each thread needs to commit all their
    transactions so that the other thread can see the changes.
    """
